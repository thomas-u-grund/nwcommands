...
**Update**: k-cores/shell numbers (`nwkcore`), maximal cliques (`nwclique`, **A**, harmonisation unit 29 - classical Bron-Kerbosch (1973) enumeration, generates each node's largest-clique-membership size plus the full cliques-by-nodes membership matrix in `r(clique_matrix)`), maximal k-plexes (`nwkplex`, **A**, harmonisation unit 34 - Seidman & Foster (1978) k-plexes via the same Bron-Kerbosch-style backtracking generalized to a whole-induced-submatrix validity check, same `r(kplex_matrix)`/per-node-summary output shape), maximal n-cliques/n-clans (`nwnclique`/`nwnclan`, **A**, harmonisation unit 36 - Luce (1950)/Mokken (1979); an n-clique turned out to be simply an ordinary clique of the "geodesic distance <= n" graph, reusing `BronKerbosch()` completely unmodified rather than a new algorithm), and maximal k-components (`nwkcomponents`, **A**, new, harmonisation unit 37 - Kanevsky (1993)/the single-level case of Moody & White (2003); required building the package's first genuine max-flow/vertex-connectivity Mata infrastructure from scratch - node-splitting max-flow reduction (Even 1979), Edmonds-Karp, Menger's theorem) are all done. **Still confirmed E**: clique census, lambda sets, factions, and the full multi-level Moody-White cohesive-blocking hierarchy (a natural follow-on now that the single-level vertex-connectivity primitive exists) — zero real implementation anywhere. `nwsimmelian` (A, Simmelian-tie/triad-embeddedness) is real and complete but is not a substitute for clique detection — a related-but-distinct concept.
