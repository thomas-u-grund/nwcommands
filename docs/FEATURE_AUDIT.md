...
**Update**: k-cores/shell numbers (`nwkcore`) and maximal cliques (`nwclique`, **A**, new, harmonisation unit 29 - classical Bron-Kerbosch (1973) enumeration, generates each node's largest-clique-membership size plus the full cliques-by-nodes membership matrix in `r(clique_matrix)`) are both done. **Still confirmed E**: clique census, n-cliques, n-clans, k-plexes, k-components, lambda sets, factions, and Moody-White cohesive blocks — zero real implementation anywhere. `nwsimmelian` (A, Simmelian-tie/triad-embeddedness) is real and complete but is not a substitute for clique detection — a related-but-distinct concept.
