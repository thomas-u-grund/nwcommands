...
6. ✅ `nwevcent` — added a `weighted` option; also found and fixed a genuine eigenvalue-search bug along the way (silently correct in practice only because of Mata's `symeigensystem()` sort order — see `docs/CERTIFICATION.md`). **Update (harmonisation unit 97)**: migrated off dense materialization after all — eigenvector centrality only needs the dominant eigenpair, not a full eigendecomposition, so sparse power iteration (with a spectral shift to handle bipartite/tree-graph convergence failure) replaces `symeigensystem()` entirely; see `docs/SPARSE_BACKEND.md`.
...
- ~~Area J (similarity/homophily/mixing/assortativity) fell between fork assignments this pass — needs a dedicated read of `nwhomophily.ado` and a clean confirmation of assortativity's absence.~~ **Closed (harmonisation phase, autonomous pass).** `nwhomophily.ado` read and its own statistical effect re-verified (harmonisation unit 119 — a prior "byte-identical output" finding was a test-methodology artifact, not a real bug). Assortativity's absence confirmed directly (no such command existed anywhere) and closed by implementing it: `nwassortativity` (harmonisation unit 121), Newman's (2002) coefficient, degree or arbitrary attribute.
...
- ~~**Compound-drop antipattern**, same class of bug fixed in `nwplot.ado` (harmonisation unit 44)~~ **Checked directly (harmonisation phase, autonomous pass) - confirmed safe, no fix needed.** `nwmovie.ado`/`nwmoviexy.ado` both have `capture drop _frame_x _frame_y` inside their own per-frame loop; read both files' code between the two `gen _frame_x`/`gen _frame_y` statements (always consecutive, no conditional between them) and the later `capture drop _frame_x _frame_y` (every loop iteration) - no branch anywhere selectively creates or destroys just one of the pair. Unlike unit 44's `_degree`/`_outdegree`/`_indegree` case (where exactly one name is *always* missing depending on directedness, guaranteeing the failure on every call), `_frame_x`/`_frame_y` genuinely always co-exist or co-not-exist, so the compound drop is safe as written.
