...
- Structural equivalence: package the existing `nwsimilar`→`nwhierarchy`→`nwdendrogram` chain as a documented "role/position analysis" workflow; add an equivalence-class-to-Stata-variable output step (mirroring `nwcomponents`' pattern). **Blocked**: this whole chain turned out to be broken in the current architecture, not merely undocumented — see `docs/CERTIFICATION.md`. Two of three layers of breakage found and fixed (`nwtomatafast`'s dead pre-2016 reference; missing `matastrict` declarations in `nwdissimilar`/`nwsimilar`'s Mata helpers); a third, precisely-reproduced-but-unresolved bug remains in how `nwset.ado`'s `mat()` option evaluates a user-defined Mata function reference. Re-scope this item as Medium-Large effort (not Small as originally estimated) and resolve the `nwset` issue before attempting the workflow-packaging/equivalence-variable work at all.
