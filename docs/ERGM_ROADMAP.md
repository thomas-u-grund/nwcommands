...
| ~~Native backend scope relaxation - "move all effects to C"~~ **DONE for wave 1 (unit 92)** | User-mandated, not profiling-gated - see this section's own "Update (unit 92)" note above | Medium (as estimated) | High | Generalized the native dispatch protocol (multi-column terms via per-slot `attridx`/`p1`/`p2`, multiple distinct attribute arrays via `native_attrmat`, replacing the original one-column/one-shared-array design) and ported the full dyad-independent attribute/factor family (`nodecov`/`nodeicov`/`nodeocov`/`absdist`/`nodematch_diff`/`nodefactor`/`nodeofactor`/`nodeifactor`/`sender`/`receiver`/`nodemix`) plus the GW-degree family (`gwdegree`/`gwodegree`/`gwidegree`). Found and fixed two real bugs: directed (OTP) `gwesp` was silently, wrongly reported native-eligible (native only ever implemented undirected/UTP - now explicitly gated); and a NULL-pointer dereference crashed the whole Stata process for any directed model lacking a `gwodegree`/`gwidegree` term (`toggle()` wrote to never-allocated `outdeg`/`indeg` arrays whenever `directed`, not just when actually needed). Measured **~29x** speedup on the exact motivating case (300-node, `edges+gwesp+nodefactor+nodecov` - "gwesp mixed with other terms"), directly, not a synthetic microbenchmark. See `docs/CERTIFICATION.md` unit 92. Still NOT native-eligible (the next waves, in roughly this priority order): `edgecov`/`hamming` (need an n x n matrix marshalled across the boundary - the attribute-array mechanism generalizes naturally, not yet done); the degree-COUNT family `degree`/`odegree`/`idegree`/`concurrent`/`degrange`/`kstar` (needs threshold-crossing logic, not a plain lookup); the shared-partner family beyond `gwesp` itself - `gwdsp`/`gwnsp`/`esp`/`dsp`/`triangle`/`ctriple`/`transitiveties`/`cyclicalties`; and directed/OTP `gwesp` itself. |
