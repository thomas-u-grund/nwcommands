...
| Incremental shared-partner cache for GWESP (`ErgmGraph::common_neighbors()` currently recomputes on demand, O(min(deg_i,deg_j)) per pair, every time `change_gwesp()`/`stat_gwesp()` needs a shared-partner count) | **Elevated to High** - see unit 81's broader benchmark results below, which pin the remaining R-vs-Stata gap specifically on GWESP-involving models (40-68x) versus GWESP-free models even at 10x the node count (~3x) | Low-Medium | Medium | A genuine, deliberate v1 simplification, not an oversight - the incremental-cache version (an `asarray()`-keyed shared-partner count, updated O(degree) per accepted toggle instead of recomputed) was independently prototyped and certified during this session's own development (2,800+ random toggles, zero mismatches against full recomputation) but not adopted into the shipped v1 file, since the simpler on-demand version is already correct and v1's own scope discipline (`docs/CERTIFICATION.md`'s own unit-68 entry) explicitly defers this exact optimization until profiling shows it matters. Swapping it in later needs no change to `change_gwesp()`'s own call signature. **Profiling now shows it matters** - this is the natural next Mata-level lever before reaching for C++, per the decision framework below. |
...
GWESP-involving models (40-68x here) are in "10x+ slower" territory, making the incremental
shared-partner cache (elevated to High priority above) the clear next Mata-level lever to try
BEFORE reaching for C++ - only if that Mata-level fix is exhausted and GWESP-heavy models
remain 10x+ slower would a C/C++ kernel for the shared-partner/change-statistic path become the
next reasonable step, and even then it should be scoped to that specific hot path rather than
the sampler generally, per the "profile before rewriting" discipline this whole unit followed.
