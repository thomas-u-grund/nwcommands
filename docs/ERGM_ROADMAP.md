...
| Default `mcmcinterval`/mixing-quality investigation for large GWESP models - a genuine open question the convergence-test fix above surfaced rather than resolved | Medium-High (affects estimate PRECISION, not just whether MCMLE terminates) | Medium (needs a proper multi-seed study, not the 5-seed exploratory probe unit 84 ran) | Medium | Unit 84's own fix correctly reports a very small effective sample size (35.7 out of 3000 raw draws) on the 500-node GWESP benchmark at DEFAULT `mcmcinterval(50)` - a correct diagnosis, but it raises whether the default itself provides adequate mixing at this scale. Two exploratory (not yet rigorous) findings: raising `mcmcinterval(500)` moved the fitted gwesp coefficient measurably closer to Statnet's own value at negligible extra cost (native execution makes a 10x larger interval still fast); a 5-seed sweep at the CURRENT default found `nodematch`'s own across-seed spread roughly 10x its own reported standard error, suggesting the existing lag-1-correction-based final SE may still understate true uncertainty for badly-mixed large models - exactly the kind of evidence the planned proper variance estimator (see the Performance section's own variance-estimator history above) should be validated against once built, alongside deciding whether `mcmcinterval`'s own default should simply be raised now that native execution has made the runtime cost of doing so much smaller than it was under the Mata-only backend. |
