...
| ~~Native backend scope relaxation - "move all effects to C"~~ **DONE for waves 1-2 (unit 92)** | User-mandated, not profiling-gated - see this section's own "Update (unit 92)" note above | Medium (as estimated) | High | Wave 1: generalized the native dispatch protocol (multi-column terms via per-slot `attridx`/`p1`/`p2`, multiple distinct attribute arrays via `native_attrmat`, replacing the original one-column/one-shared-array design) and ported the full dyad-independent attribute/factor family (`nodecov`/`nodeicov`/`nodeocov`/`absdist`/`nodematch_diff`/`nodefactor`/`nodeofactor`/`nodeifactor`/`sender`/`receiver`/`nodemix`) plus the GW-degree family (`gwdegree`/`gwodegree`/`gwidegree`). Found and fixed two real bugs: directed (OTP) `gwesp` was silently, wrongly reported native-eligible (native only ever implemented undirected/UTP - now explicitly gated); and a NULL-pointer dereference crashed the whole Stata process for any directed model lacking a `gwodegree`/`gwidegree` term. Measured **~29x** speedup on the exact motivating case (300-node, `edges+gwesp+nodefactor+nodecov`). Wave 2: the full degree-COUNT family - `degree`/`odegree`/`idegree`/`concurrent`/`kstar`/`ostar`/`istar`/`degrange`/`odegrange`/`idegrange` - needed no new attribute plumbing, just wave 1's own `outdeg`/`indeg` bookkeeping plus two direct C ports of existing Mata combinatorics helpers (`_ergm_choose()`, `_ergm_inrange()`). Every dyad-independent AND degree-based term family is now fully native. See `docs/CERTIFICATION.md` unit 92 for both waves' complete evidence trail. Still NOT native-eligible (the remaining waves, in roughly this priority order): `edgecov`/`hamming` (need an n x n matrix marshalled across the boundary - the attribute-array mechanism generalizes naturally, not yet done); the shared-partner family beyond `gwesp` itself - `gwdsp`/`gwnsp`/`esp`/`dsp`/`triangle`/`ctriple`/`transitiveties`/`cyclicalties`; and directed/OTP `gwesp` itself. |
