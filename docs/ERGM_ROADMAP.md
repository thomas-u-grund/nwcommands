...
moderately dense graph, single-threaded Mata, results are per-call/per-step averages):
...
| One MCMC step, **uniform** proposal | 5.80 us/step | The proposal itself is O(1) (a single random-index unranking, no graph traversal). |
| One MCMC step, **TNT** proposal | 1072.65 us/step | **~185x slower than uniform, at n=200** - overwhelmingly dominated by `ergm_propose_tnt()`'s own `G.all_ties()` call, which materializes the FULL tie list (O(n+nties)) on every single proposal, not just when an edge-removal is actually selected. This is the dominant, measured, single largest cost in the entire sampler at any but the smallest network sizes - confirming (not merely predicting) `unw_ergm.do`'s own already-disclosed "maintain a live edge list for O(1) TNT edge-picks" item, now promoted from a hypothesis to a benchmarked finding. |

| Item | Usage | Effort | Infra value | Notes |
|---|---|---|---|---|
| **Live edge list for TNT's own edge-pick** (`ergm_propose_tnt()` currently calls `G.all_ties()`, O(n+nties), on every single proposal just to pick one uniformly random existing edge) | **High - this is the measured, dominant bottleneck** (185x the cost of the uniform proposal at n=200; the gap widens with n and density, since `all_ties()`'s own cost grows with both while the rest of a proposal stays O(1)) | Low-Medium | High | Maintain a live, incrementally-updated list/array of current edges on `ErgmGraph` itself (append on toggle-add, swap-and-truncate on toggle-remove - the standard O(1) "swap with last element" removal trick for an unordered array), so TNT's own edge-pick becomes O(1): `ties[ceil(runiform()*count)]` instead of rebuilding the entire list first. No other proposal or term needs to change - this is entirely internal to `ErgmGraph`/`ergm_propose_tnt()`. Now the clear TOP performance-track item, ahead of a full C/C++ kernel (below), since it is a self-contained Mata-level fix with high measured payoff and low implementation risk. |
| C/C++ MCMC kernel (Stata plugin, `stplugin.h`/`stplugin.c`) | High (for networks beyond a few hundred nodes at realistic MCMC sample sizes, once the Mata-level TNT fix above is exhausted) | High | High | v1 deliberately ships a pure-Mata kernel (associative-array-backed sparse graph, per PART XXXVI's own "do not move ordinary functionality into C merely for ideological reasons" and PART XXXVII's own scope discipline) and profiles it first (done: see the benchmark table above). The measured hot loop is TNT's own `all_ties()` call, not per-toggle change-statistic evaluation as originally hypothesized before benchmarking - fix that at the Mata level first (item above); a C kernel remains the right next step once Mata-level fixes are exhausted and networks/sample sizes genuinely demand it. Build as a single "run N MCMC steps, return sampled statistics" native call (see architecture doc's "Stata/Mata to C boundary" principle) - never a per-toggle Stata<->C round trip. |
