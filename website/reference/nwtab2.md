...
Teh command produces the network version of `tabulate twoway` for two networks. It shows the overlap of ties for two networks that share the same nodes (see [nodeid](nodeid.md)) on the dyadic level. The command essentially transforms *netname1* and *netname2* in edgelist format (see [nwtoedge](nwtoedge.md)) and runs a normal `tabulate twoway`, hence, all `tabulate2_options` can be used as well.
...
- [one-way nwtabulate](nwtab1.md), [two-way nwtabulate attribute](nwtab3.md), [nwcorrelate](nwcorrelate.md), [nwqap](nwqap.md), `tabulate`
