...
program nwmoviexy
	...
	// confirmed directly via probe before this fix (see the identical
	// fix and its own comment in nwmovie.ado, this file's near-twin),
	// meaning nwmoviexy could never actually complete a single call.
	// Replaced with nw_syntax (other() to avoid clobbering this
	// program's own same-named locals). _nwsyntax_other's own
	// self-loop-removal side effect (also unconditionally broken, same
	// nw_mata`id' reference) is dropped rather than reimplemented blind
	// - see nwmovie.ado's own comment for the full reasoning.
	local sizeCheck = 0
	...
	// repeating its last element - replicated directly since nw_syntax
