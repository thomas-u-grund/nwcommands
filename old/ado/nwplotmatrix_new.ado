...
program nwplotmatrix
	syntax [anything(name=netname)],[ * sortby(varlist) group(string) label(varname) ylabel(string) xlabel(string) BAckground(string) labelopt(string) legendopt(string asis) COlorpalette(string) LColor(string) legend(string asis) tievalue tievalueopt(string) ]
	unw_defs
	nw_syntax `netname'
	...
	qui plotmatrix, `xlinecmd' `ylinecmd' label(`label') legend(off) `freq' tievalueopt(`tievalueopt') ylabel(, angle(0) `ylabel') xlabel(, angle(90) `xlabel') aspect(1) mat(`tempmat') background(`background') color(`colorpalette') lcolor(`lcolor') `options'  
	restore
end
